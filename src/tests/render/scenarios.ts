...
for (const scenario of immersiveRaw) {
    const maxDuraction = scenario.reduce((acc, viewRequest) => Math.max(acc, viewRequest.duration), 0) * 100;
    for (const viewRequest of scenario) {
        const duration = viewRequest.duration * 100;
        const easing = viewRequest.easing;
...
export const cases = { common, memory, beauty, lemesos, perf, moscow, immersive };
...
function setPitch(pitch, options?, delay?) {
    if (typeof options !== 'object') {
        delay = options;
    }
    delay = delay === undefined ? 3000 : delay;
